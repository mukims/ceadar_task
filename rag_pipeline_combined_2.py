...
def run_pipeline(cfg: RAGConfig, query: str) -> None:
    ...
    # Report which files were ingested.  Each Document stores a source or filename
    # in its metadata; we collect and print these once at the start for
    # transparency when multiple files are processed.
    try:
        file_names = []
        for doc in raw_docs:
...
def parse_args() -> argparse.Namespace:
    ...
    # Default to a CPU-friendly embedding model; override as needed.
    parser.add_argument(
        "--embedding-model",
        type=str,
